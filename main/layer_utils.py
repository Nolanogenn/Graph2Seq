...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=reuse):
        if use_cudnn:
            inputs = tf.transpose(a=input_reps, perm=[1, 0, 2])
...
def dropout_layer(input_reps, dropout_rate, is_training=True):
    if is_training:
        output_repr = tf.nn.dropout(input_reps, rate=(1 - (1 - dropout_rate)))
    else:
...
def euclidean_distance(y1, y2, eps=1e-6):
    distance = tf.sqrt(tf.maximum(tf.reduce_sum(input_tensor=tf.square(y1 - y2), axis=-1), eps))
    return distance
...
def cross_entropy(logits, truth, mask=None):
    ...
    xdev = tf.subtract(logits, tf.expand_dims(tf.reduce_max(input_tensor=logits, axis=1), -1))
    log_predictions = tf.subtract(xdev, tf.expand_dims(tf.math.log(tf.reduce_sum(input_tensor=tf.exp(xdev),axis=-1)),-1))
...
#     feat_dim = input_shape[2]
    ...
    with tf.compat.v1.variable_scope(scope or "projection_layer"):
        full_w = tf.compat.v1.get_variable("full_w", [input_size, output_size], dtype=tf.float32)
        full_b = tf.compat.v1.get_variable("full_b", [output_size], dtype=tf.float32)
        outputs = activation_func(tf.compat.v1.nn.xw_plus_b(in_val, full_w, full_b))
...
#     feat_dim = input_shape[2]
    ...
    with tf.compat.v1.variable_scope(scope or "highway_layer"):
        ...
        full_b = tf.compat.v1.get_variable("full_b", [output_size], dtype=tf.float32)
        trans = activation_func(tf.compat.v1.nn.xw_plus_b(in_val, full_w, full_b))
...
def collect_probs(probs, positions):
    ...
    batch_size = tf.shape(input=probs)[0]
    pair_size = tf.shape(input=positions)[1]
    batch_nums = tf.range(0, limit=batch_size) # shape (batch_size)
...
def calcuate_attention(in_value_1, in_value_2, feature_dim1, feature_dim2, scope_name='att',
    ...
    input_shape = tf.shape(input=in_value_1)
    batch_size = input_shape[0]
    len_1 = input_shape[1]
    len_2 = tf.shape(input=in_value_2)[1]
    ...
    with tf.compat.v1.variable_scope(scope_name):
        ...
        if att_type == 'additive':
            ...
            atten_value = tf.reduce_sum(input_tensor=atten_value, axis=-1)
        ...
        if remove_diagnoal:
            ...
            diagnoal = 1.0 - tf.linalg.tensor_diag(diagnoal)  # [len1, len1]
...
def compute_gradients(tensor, var_list):
  grads = tf.gradients(ys=tensor, xs=var_list)
