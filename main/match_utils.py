...
def cosine_distance(y1, y2):
    # y1 [....,a, 1, d]
    # y2 [....,1, b, d]
    cosine_numerator = tf.reduce_sum(input_tensor=tf.multiply(y1, y2), axis=-1)
    y1_norm = tf.sqrt(tf.maximum(tf.reduce_sum(input_tensor=tf.square(y1), axis=-1), eps))
    y2_norm = tf.sqrt(tf.maximum(tf.reduce_sum(input_tensor=tf.square(y2), axis=-1), eps))
...
def cal_maxpooling_matching(passage_rep, question_rep, decompose_params):
    ...
    return tf.concat(axis=2, values=[tf.reduce_max(input_tensor=matching_matrix, axis=2), tf.reduce_mean(input_tensor=matching_matrix,
...
def cross_entropy(logits, truth, mask):
    ...
    xdev = tf.sub(logits, tf.expand_dims(tf.reduce_max(input_tensor=logits, axis=1), -1))
    log_predictions = tf.sub(xdev, tf.expand_dims(tf.math.log(tf.reduce_sum(input_tensor=tf.exp(xdev), axis=-1)), -1))
    #     return -T.sum(targets * log_predictions)
    result = tf.multiply(tf.multiply(truth, log_predictions), mask)  # [batch_size, passage_len]
    return tf.multiply(-1.0, tf.reduce_sum(input_tensor=result, axis=-1))  # [batch_size]


def highway_layer(in_val, output_size, scope=None):
    # in_val: [batch_size, passage_len, dim]
    input_shape = tf.shape(input=in_val)
...
def cal_max_question_representation(question_representation, atten_scores):
    atten_positions = tf.argmax(input=atten_scores, axis=2, output_type=tf.int32)  # [batch_size, passage_len]
...
def multi_perspective_match(feature_dim, repres1, repres2, is_training=True, dropout_rate=0.2,
    ...
    '''
        ...
        :return:
    '''
    input_shape = tf.shape(input=repres1)
    batch_size = input_shape[0]
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=reuse):
        ...
        if options['with_mp_cosine']:
            mp_cosine_params = tf.compat.v1.get_variable("mp_cosine", shape=[options['cosine_MP_dim'], feature_dim],
                                               dtype=tf.float32)
...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths,
    ...
    with tf.compat.v1.variable_scope(scope or "match_passage_with_question"):
        ...
        all_question_aware_representatins.append(tf.reduce_max(input_tensor=relevancy_matrix, axis=2, keepdims=True))
        all_question_aware_representatins.append(tf.reduce_mean(input_tensor=relevancy_matrix, axis=2, keepdims=True))
        dim += 2
        if with_full_match:
            if forward:
            ...
            passage_len = tf.shape(input=passage_reps)[1]
        ...
        if with_maxpool_match:
            maxpooling_decomp_params = tf.compat.v1.get_variable("maxpooling_matching_decomp",
...
def bilateral_match_func(in_question_repres, in_passage_repres,
    ...
    if is_training:
        question_aware_representatins = tf.nn.dropout(question_aware_representatins, rate=(1 - (1 - options['dropout_rate'])))
        passage_aware_representatins = tf.nn.dropout(passage_aware_representatins, rate=(1 - (1 - options['dropout_rate'])))
    ...
    if options['with_aggregation_highway']:
        with tf.compat.v1.variable_scope("aggregation_highway"):
            agg_shape = tf.shape(input=aggregation_representation)
